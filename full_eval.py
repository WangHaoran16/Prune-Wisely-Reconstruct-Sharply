...
if not args.skip_training:
    ...
    for scene in mipnerf360_outdoor_scenes:
        ...
        os.system("python train_prune_DoG.py -s " + source + " -i images_4 -m " + args.output_path + "/" + scene + common_args)
    for scene in mipnerf360_indoor_scenes:
        source = args.mipnerf360 + "/" + scene
        os.system("python train_prune_DoG.py -s " + source + " -i images_2 -m " + args.output_path + "/" + scene + common_args)
    m360_timing = (time.time() - start_time)/60.0

    start_time = time.time()
    for scene in tanks_and_temples_scenes:
        source = args.tanksandtemples + "/" + scene
        os.system("python train_prune_DoG.py -s " + source + " -m " + args.output_path + "/" + scene + common_args)
    ...
    with open(os.path.join(args.output_path, "timing.txt"), "w") as file:
        file.write(
            ...
            f"tandt: {tandt_timing} minutes\n"
            f"db: {db_timing} minutes\n"
        )
...
if not args.skip_rendering:
    ...
    for scene, source in zip(all_scenes, all_sources):
        model_path = f"{args.output_path}/{scene}"

        os.system(
            f'python render_DoG.py --iteration 40000 '
            f'-s "{source}" '
            f'-m "{model_path}"{common_args}'
        )

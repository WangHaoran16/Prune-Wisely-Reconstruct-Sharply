...
RasterizeGaussiansCUDA(
	...
	const torch::Tensor& dog_opacity,
	const torch::Tensor& dog_scales,
  ...
  if(P != 0)
  {
	  ...
	  auto tup = CudaRasterizer::Rasterizer::forward(
		...
		sh.contiguous().data_ptr<float>(),
			...
			dog_opacity.contiguous().data_ptr<float>(),
			dog_scales.contiguous().data_ptr<float>(),
			scale_modifier,
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
 RasterizeGaussiansBackwardCUDA(
	...
	const torch::Tensor& scales,
	const torch::Tensor& dog_opacities,
	const torch::Tensor& dog_scales,
	const torch::Tensor& rotations,
	const float scale_modifier,
	const torch::Tensor& cov3D_precomp,
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_ddogconic = torch::zeros({P, 2, 2}, means3D.options());
  torch::Tensor dL_dopacity = torch::zeros({P, 1}, means3D.options());

  torch::Tensor dL_dcov3D = torch::zeros({P, 6}, means3D.options());
  torch::Tensor dL_ddogcov3D = torch::zeros({P, 6}, means3D.options());
  torch::Tensor dL_ddc = torch::zeros({P, 1, 3}, means3D.options());
  torch::Tensor dL_dsh = torch::zeros({P, M, 3}, means3D.options());
  torch::Tensor dL_dscales = torch::zeros({P, 3}, means3D.options());
  torch::Tensor dL_ddogopacities = torch::zeros({P, 1}, means3D.options());
  torch::Tensor dL_ddogscales = torch::zeros({P, 3}, means3D.options());
  ...
  {  
	  ...
	  sh.contiguous().data<float>(),
		  ...
		  dog_opacities.data_ptr<float>(),
		  dog_scales.data_ptr<float>(),
		  scale_modifier,
	  ...
	  dL_dout_invdepth.contiguous().data<float>(),
		  ...
		  dL_ddogconic.contiguous().data<float>(),
		  dL_dopacity.contiguous().data<float>(),
		  dL_ddogopacities.contiguous().data<float>(),
	  dL_dcolors.contiguous().data<float>(),
	  dL_dinvdepths.contiguous().data<float>(),
		  dL_dmeans3D.contiguous().data<float>(),
		  dL_dcov3D.contiguous().data<float>(),
		  dL_ddogcov3D.contiguous().data<float>(),
	  dL_ddc.contiguous().data<float>(),
		  dL_dsh.contiguous().data<float>(),
		  dL_dscales.contiguous().data<float>(),
		  dL_ddogscales.contiguous().data<float>(),
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_ddc, dL_dsh, dL_dscales, dL_ddogopacities, dL_ddogscales, dL_drotations, dL_dG2);
}

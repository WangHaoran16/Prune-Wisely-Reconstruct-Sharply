	...
	bool antialiasing)
{
	...
	float3 dog_cov = cov;
	const bool use_dog = dog_opacities != nullptr && dog_scales != nullptr && dog_conic_opacity != nullptr;
	if (use_dog)
	{
		computeCov3D(dog_scales[idx], scale_modifier, rotations[idx], dog_cov3Ds + idx * 6);
		dog_cov = computeCov2D(p_orig, focal_x, focal_y, tan_fovx, tan_fovy, dog_cov3Ds + idx * 6, viewmatrix);
	}
	...
	if (use_dog)
	{
		float dog_mid = 0.5f * (dog_cov.x + dog_cov.z + 2.0f * h_var);
		float dog_det = (dog_cov.x + h_var) * (dog_cov.z + h_var) - dog_cov.y * dog_cov.y;
		float dog_lambda1 = dog_mid + sqrt(max(0.1f, dog_mid * dog_mid - dog_det));
		float dog_lambda2 = dog_mid - sqrt(max(0.1f, dog_mid * dog_mid - dog_det));
		float dog_radius = ceil(3.f * sqrt(max(dog_lambda1, dog_lambda2)));
		my_radius = max(my_radius, dog_radius);
		getRect(point_image, my_radius, rect_min, rect_max, grid);
	}
	...
	if (use_dog)
	{
		const float dog_det_cov = dog_cov.x * dog_cov.z - dog_cov.y * dog_cov.y;
		dog_cov.x += h_var;
		dog_cov.z += h_var;
		const float dog_det_cov_plus_h_cov = dog_cov.x * dog_cov.z - dog_cov.y * dog_cov.y;
		float dog_h_convolution_scaling = 1.0f;
		if (antialiasing)
			dog_h_convolution_scaling = sqrt(max(0.000025f, dog_det_cov / dog_det_cov_plus_h_cov));
		const float dog_det_inv = 1.f / dog_det_cov_plus_h_cov;
		float3 dog_conic = { dog_cov.z * dog_det_inv, -dog_cov.y * dog_det_inv, dog_cov.x * dog_det_inv };
		dog_conic_opacity[idx] = { dog_conic.x, dog_conic.y, dog_conic.z, dog_opacities[idx] * dog_h_convolution_scaling };
	}
	...
	float* __restrict__ invdepth)
{
	...
	__shared__ float4 collected_dog_conic_opacity[BLOCK_SIZE];
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			if (dog_conic_opacity)
				collected_dog_conic_opacity[block.thread_rank()] = dog_conic_opacity[coll_id];
		}
		block.sync();
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float alpha_o = min(0.99f, con_o.w * exp(power));
			float dog_alpha = 0.0f;
			if (dog_conic_opacity)
			{
				float4 dog_con_o = collected_dog_conic_opacity[j];
				float dog_power = -0.5f * (dog_con_o.x * d.x * d.x + dog_con_o.z * d.y * d.y) - dog_con_o.y * d.x * d.y;
				if (dog_power <= 0.0f)
					dog_alpha = min(0.99f, dog_con_o.w * exp(dog_power));
			}
			float alpha = alpha_o - dog_alpha;
			if (fabsf(alpha) < 1.0f / 255.0f)
				continue;

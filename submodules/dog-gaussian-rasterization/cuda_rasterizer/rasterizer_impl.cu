	...
	int2* rects)
{
	...
	if (radii[idx] > 0)
	{
		...
		if(rects == nullptr)
			...
			const float4 dog_co = dog_conic_opacity[idx];
			const float opacity_threshold = 1.0f / 255.0f;
			const float opacity_factor_threshold = logf(co.w / opacity_threshold);
			const float dog_opacity_factor_threshold = logf(fabsf(dog_co.w) / opacity_threshold);
		...
		for (int y = rect_min.y; y < rect_max.y; y++)
		{
			for (int x = rect_min.x; x < rect_max.x; x++)
			{
				...
				const float max_dog_opac_factor = max_contrib_power_rect_gaussian_float<BLOCK_X-1, BLOCK_Y-1>(dog_co, xy, tile_min, tile_max, max_pos);
				
				uint64_t key = y * grid.x + x;
				key <<= 32;
				key |= *((uint32_t*)&depths[idx]);
				if (max_opac_factor <= opacity_factor_threshold || max_dog_opac_factor <= dog_opacity_factor_threshold) {
...
CudaRasterizer::GeometryState CudaRasterizer::GeometryState::fromChunk(char*& chunk, size_t P)
{
	...
	obtain(chunk, geom.dog_cov3D, P * 6, 128);
	obtain(chunk, geom.conic_opacity, P, 128);
	obtain(chunk, geom.dog_conic_opacity, P, 128);
